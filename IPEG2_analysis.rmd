...
Bayes_NR<- brm(data=IPEG2.df, family=cumulative("logit"),
                      formula= bf(Help~Conditions+HT_sex_c+HT_age_c+(1|participant)+
...
set.seed(67);posterior_NR<- posterior_samples(Bayes_NR) 
# Mu2
Bayes_gen_NR<- brm(data=IPEG2.df, family=cumulative("logit"),
                      formula= bf(Help~Match_condition+HT_sex_c+HT_age_c+(1|participant)+
                      (1|Sim_SID)+(1|HTFace_ID)+(1|STFace_ID)+(1|HS_ID)) +
                   lf(disc~ 0+Match_condition, cmc=F),
                  prior= c(prior(normal (0,1), class=b),
                           prior(normal(0,1.2), class=Intercept),
                           prior(exponential(1), class= sd)),
                  iter=7000,warmup =2000, chains=8, cores=8, seed=20, sample_prior=T,save_all_pars = T))
set.seed(67);posterior_gen_NR<- posterior_samples(Bayes_gen_NR)

# Mu3
Bayes_mu3_NR<- brm(data=IPEG2.df, family=cumulative("logit"),
                      formula= bf(Help~ORM_code+HT_sex_c+HT_age_c+(1|participant)+
                      (1|Sim_SID)+(1|HTFace_ID)+(1|STFace_ID)+(1|HS_ID)) +
                   lf(disc~ 0+ORM_code, cmc=F),
                  prior= c(prior(normal (0,1), class=b),
                           prior(normal(0,1.2), class=Intercept),
                           prior(exponential(1), class= sd)),
                  iter=7000,warmup = 2000, chains=8, cores=8, seed=20, sample_prior=T,save_all_pars = T)
set.seed(67);posterior_mu3_NR<- posterior_samples(Bayes_mu3_NR)
...
m2_NR<-(posterior_gen_NR[, "b_Match_conditionHTMHSM"] > 0) & 
  ...
  (posterior_gen_NR[, "b_Match_conditionHTPHSM"] < posterior_gen_NR[, "b_Match_conditionHTMHSP"]) &
  (posterior_gen_NR[, "b_Match_conditionHTPHSP"]*100 > posterior_gen_NR[, "b_Match_conditionHTMHSP"])
...
# BFs
print(paste0("M0/M3: ", BF_M0_vs_M3_NR))
print(paste0("M0/M1: ", BF_M0_vs_M1_NR))
